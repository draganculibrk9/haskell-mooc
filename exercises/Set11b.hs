...
appendAll ref values = do
    forM_ values (\x -> modifyIORef ref (++x))
...
swapIORefs r1 r2 = do
    r1_value <- readIORef r1
    r2_value <- readIORef r2
    writeIORef r1 r2_value
    writeIORef r2 r1_value
...
hSelectLines h nums = do
    content <- hFetchLines h
    return [content !! (i-1) | i <- nums]
...
interact' f state = do
    line <- getLine
    let (continue,value,newState) = f (line,state)
