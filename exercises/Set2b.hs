...
leftpad s len = if length s >= len
                then s
                else " " ++ leftpad s (len - 1)
...
decrementTil1 :: Integer -> String
decrementTil1 1 = "1... "
decrementTil1 i = show i ++ "... " ++ decrementTil1 (i - 1)

countdown :: Integer -> String
countdown n = "Ready! " ++ decrementTil1 n ++ "Liftoff!" 
...
smallestDivisor' k n = if n `rem` k == 0
                       then k
                       else if k ^ 2 > n
                            then n
