...
fromList [] = Empty
fromList [single] = Pair (fst single) (snd single) Empty
...
toNat i = if i < 0
          then Nothing
          else Just (toNatHelper i Zero)

toNatHelper :: Int -> Nat -> Nat
toNatHelper 0 accumulated = accumulated
toNatHelper current accumulated = toNatHelper (current - 1) (PlusOne accumulated)
...
fromBin :: Bin -> Int
fromBin b = fromBinHelper b 0 0

fromBinHelper :: Bin -> Int -> Int -> Int
fromBinHelper End degree sum = sum
fromBinHelper (O rest) degree sum = fromBinHelper rest (degree + 1) sum
fromBinHelper (I rest) degree sum = fromBinHelper rest (degree + 1) (sum + 2 ^ degree)

toBin :: Int -> Bin
toBin i = if i > 1
          then if i `mod` 2 == 0
               then O (toBin (i `div` 2))
