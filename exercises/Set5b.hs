...
valAtRoot (Node root l r) = Just root
...
treeMax Empty = 0
treeMax (Node root l r) = maximum [root, treeMax l, treeMax r]
...
walk (first:rest) (Node v l r) = if first == StepL
                                 then walk rest l
                                 else walk rest r
...
set [] value (Node root l r) = Node value l r
set (first:rest) value (Node root l r) = if first == StepL
                                         then Node root (set rest value l) r
                                         else Node root l (set rest value r)
