...
firstHalf s = take (half s) s
              where half s = if even (length s)
                             then ((length s) `div` 2)
...
capitalizeFirst (h:rest) = (toUpper h):rest
...
while check update value = if not (check value)
                           then value
...
joinToLength l xs = filter (\v -> length v == l) [ first ++ second | first <- xs, second <- xs ]
...
multiCompose [] v = v
multiCompose fs v = multiCompose (init fs) ((head (reverse fs)) v)
