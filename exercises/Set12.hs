...
fmap2 fun x = fmap (\v -> fmap fun v) x 
fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 fun x = fmap (\v -> fmap2 fun v) x
...
count value values = foldr (countValue value) 0 values

countValue :: Eq a => a -> a -> Int -> Int
countValue value current counts = if current == value
                                  then counts + 1
                                  else counts
...
inBoth f g = foldr (inBothFoldables g) [] f

inBothFoldables :: (Foldable g, Eq a) => g a -> a -> [a] -> [a]
inBothFoldables values value inBoth = if elem value values
                             then value:inBoth
                             else inBoth
...
instance Foldable List where
  ...
  foldr f initialValue (LNode value rest) = f value (foldr f initialValue rest)
...
instance Foldable TwoList where
  ...
  foldr f initialValue (TwoNode v1 v2 rest) = f v1 (f v2 (foldr f initialValue rest)) 
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node v lt rt) = Node (f v) (fmap f lt) (fmap f rt)

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node v lt rt) = (sumTree lt) <> v <> (sumTree rt)
