...
member value (Set []) = False
member value (Set (first:rest)) = first == value || (member value (Set rest))

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add value (Set values) = if not (member value (Set values))
                         then Set (sort (value:values))
                         else Set values
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  Set a1 <> Set a2 = Set (sort (nub(a1 ++ a2)))
...
passwordAllowed password (ContainsSome characters) = any (==True) [c `elem` password | c <- characters]
passwordAllowed password (DoesNotContain characters) = not (passwordAllowed password (ContainsSome characters))
...
operation _ a b = Multiplication a b
