...
countHelper (Just value) sum = sum
...
maxHelper current max = if max > current
                        then max
                        else current
...
slHelper value state = (fst state + value, snd state + 1) 
...
concatHelper :: [a] -> [a] -> [a]
concatHelper current concatenated = current ++ concatenated
...
largestHelper current largest = if null largest
                                then [current]
                                else if current /= head largest
                                        then if current > head largest
                                                then [current]
                                                else largest
...
headHelper current (Just head) = Just current
...
lastHelper current (Just head) = Just head
